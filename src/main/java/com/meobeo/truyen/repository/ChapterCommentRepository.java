...
public interface ChapterCommentRepository extends JpaRepository<ChapterComment, Long> {
        ...
        @Query("SELECT COUNT(cc) FROM ChapterComment cc " +
                        "JOIN cc.chapter c " +
                        "WHERE cc.story.id = :storyId AND c.chapterNumber = :chapterNumber")
        Long countByStoryIdAndChapterNumber(@Param("storyId") Long storyId,
                        @Param("chapterNumber") Integer chapterNumber);
         ...
         */
        @Query("SELECT COUNT(cc) FROM ChapterComment cc " +
                        "WHERE cc.user.id = :userId AND cc.updatedAt >= :sinceTime " +
                        "AND cc.updatedAt IS NOT NULL")
        Long countRecentUpdatesByUser(
                        @Param("userId") Long userId,
                        @Param("sinceTime") java.time.LocalDateTime sinceTime);

        /**
         * Tìm comment theo ID với thông tin user và chapter
         */
        @Query("SELECT cc FROM ChapterComment cc " +
                        "JOIN FETCH cc.user u " +
                        "JOIN FETCH cc.chapter c " +
                        "JOIN FETCH cc.story s " +
                        "WHERE cc.id = :commentId")
        java.util.Optional<ChapterComment> findByIdWithDetails(@Param("commentId") Long commentId);
}
